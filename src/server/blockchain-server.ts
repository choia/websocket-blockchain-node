...
export class BlockchainServer extends MessageServer<Message> {
  ...
  protected handleMessage(sender: WebSocket, message: Message): void {
    switch (message.type) {
      case MessageTypes.GetLongestChainRequest:
        return this.handleGetLongesetChainRequest(sender, message)
  ...
  private handleGetLongesetChainRequest(requestor: WebSocket, message: Message): void {
    if (this.clientIsNotAlone) {
      ...
      this.broadcastExcept(request, message)
    } else {
